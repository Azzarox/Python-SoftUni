# my code solo

def read_matrix(is_test=False):
    if is_test:
        return [
            ['A', 'B', 'C'],
            ['D', 'E', 'F'],
            ['X', '!', '@'],
        ]
    else:
        size = int(input())
        matrix = []
        for row_index in range(size):
            row = [el for el in input()]
            matrix.append(row)
        return matrix


def find_symbol(matrix, symb):
    for row_index in range(len(matrix)):
        for column_index in range(len(matrix)):
            if matrix[row_index][column_index] == symb:
                return row_index, column_index
    return None


def print_result(result, symb):
    if result:
        print(result)
    else:
        print(f'{symb} does not occur in the matrix')


matrix = read_matrix()
symbol = input()
res = find_symbol(matrix, symbol)
print_result(res, symbol)


# TODO: от презентацията
# def read_matrix(is_test=False):
#     if is_test:
#         return [
#             ['A', 'B', 'C'],
#             ['D', 'E', 'F'],
#             ['X', '!', '@'],
#         ]
#     else:
#         size = int(input())
#         matrix = []
#         for row_index in range(size):
#             row = [x for x in input()]
#             matrix.append(row)
#
#         return matrix
#
#
# def find_symbol_in_matrix(matrix, symbol):
#     for row_index in range(len(matrix)):
#         if symbol in matrix[row_index]:
#             return row_index, matrix[row_index].index(symbol)
#         # for col_index in range(len(matrix[row_index])): # range(len(matrix[row_index])) -> в случаят може да ползваме само range(len(matrix)) защото е квадратна матрица
#         #     current_symbol = matrix[row_index][col_index]
#         #     if current_symbol == symbol:
#         #         return (row_index, col_index)
#     return None
#
#
# def print_result(result, symbol):
#     if result:
#         print(result)
#     else:
#         print(f'{symbol} does not occur in the matrix')
#
#
# matrix = read_matrix(is_test=True)
# symbol = '1'
#
# print_result(find_symbol_in_matrix(matrix, symbol), symbol)